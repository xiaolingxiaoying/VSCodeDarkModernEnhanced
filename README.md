...
编辑器交互颜色通过 `mappings/sublime_ui_overrides.json` 保留旧版 Sublime 配色的设置：当前行背景 `#2A2D2E`、活动行号文字 `#CCCCCC`、光标 `#AEAFAD`、括号匹配下划线 `#D7BA7D`、选区及边框 `#264F78`。这些覆盖项同样会记录在 `theme-build-report.json` 中，重新构建不会丢失。
